...
export function createLiveIntakeAdapter(url: string): IntakeAdapter {
  ...
  return {
    ...
    async updateDraft(jobId, input) {
      await client.mutation(api.agency.updateBrief, {
    ...
    async launch(jobId) {
      await client.mutation(api.agency.startJob, {
        jobId: jobId as Id<"jobs">,
  ...
  return {
    ...
    async listJobs() {
      return (await client.query(
        api.agency.getControlRoomJobs,
        {},
      )) as ControlJob[];
    },
    async getJob(id) {
      return (await client.query(api.agency.getControlRoomJob, {
        jobId: id as Id<"jobs">,
      })) as ControlRoomDetail | null;
    },
    async approveArtifact(id) {
      await client.mutation(api.agency.approveArtifact, {
        artifactId: id as Id<"artifacts">,
      });
    },
    async requestChanges(id, note) {
      await client.mutation(api.agency.requestArtifactRevision, {
        artifactId: id as Id<"artifacts">,
        note,
      });
    },
    async retryTask(id) {
      await client.mutation(api.agency.retryTask, {
        taskId: id as Id<"tasks">,
      });
    },
    async publish(job) {
      await client.mutation(api.agency.publishJob, {
        jobId: job.id as Id<"jobs">,
        publicBaseUrl: window.location.origin,
      });
    },
    async unpublish(job) {
      await client.mutation(api.agency.unpublishJob, {
        jobId: job.id as Id<"jobs">,
      });
    },
  };
...
export async function fetchPublishedSite(url: string, slug: string) {
  const client = createAgencyClient(url);
  return (await client.query(api.agency.getPublishedSite, {
    slug,
  })) as PublishedSite | null;
}
